...
const Header = () => {
  ...
  return (
    <header className="px-8 py-4 dark:bg-darkMode bg-white fixed z-10 w-full">
      <div className="flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center space-x-8 text-sm sm:text-base">
          {isAuthenticated ? (
            ...
            <Link to="/login" className="font-semibold text-slate-900 dark:text-white">
              로그인
            </Link>
          )}
          <button onClick={toggleDarkMode} className="p-2 text-black dark:bg-darkMode dark:text-white">
            {darkMode ? <SunIcon className="h-5 sm:h-6 w-5 sm:w-6" /> : <MoonIcon className="h-5 sm:h-6 w-5 sm:w-6" />}
          </button>
        </div>
      </div>
    </header>
