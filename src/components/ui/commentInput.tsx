  ...
  return (
    <>
      <div>
        ...
        // 댓글 등록
        type == 'add' ? (
          <Button className="w-full" onClick={commentRegist}>등록</Button>
        ) : 
        // 대댓글 등록
        type === 'replyAdd' ? (
          <Button className="w-full" onClick={commentReplyAdd}>등록</Button>
        ) : (
        // 댓글 수정
          <Button className="w-full" onClick={commentModify}>수정</Button>
        )}
      </div>
    </>
  );
