  ...
  return (
    <div className={cn('flex flex-col flex-none w-48', className)}>
      {currentMenu.map(item => (
        <a
          key={item.value}
          className={`gap-2 font-semibold text-left items-center px-6 py-3 rounded-lg flex cursor-pointer text-sm sm:text-base ${
            isActive(item.value) ? 'text-slate-900 dark:text-white' : 'text-slate-500'
          }`}
